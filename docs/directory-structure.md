...
- **左レールは固定幅ではなく中身に追従します** (`auto`)。ナビのラベルは左寄せなので、
  固定幅のトラックに乗せると文字の右側に余白が残り、そのぶん本文との間隔が右レール側より
  広く見えます。`auto` なら箱が一番長いラベルに張り付くので、左右の間隔が揃います。
  ラベルを足したり伸ばしたりすると左レールの幅もそのぶん動きます。
...
- 左レールは `lg` (1024px) 以上で縦レール、それ未満では 1 カラムに畳まれた grid の
  ...
  px で焼き込まずに済みます。
- 右レールは `xl` (1280px) 以上でのみ出ます。3 段が収まる幅は 1085px 前後
  (左レール 157 + 32 + 624 + 32 + 240) ですが、その幅ちょうどで出すとレールがウィンドウの
  端に貼り付くため、左右に 約 98px 残る `xl` まで待ちます。
